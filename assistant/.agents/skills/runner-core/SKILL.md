...
description: >-
  ...
  Коммандос, Нажать, ВнестиЗначение, Выбрать и средства чтения таблиц.
---
...
4. Перед взаимодействием с формой, установи контекст выполнения через `Здесь("<ЗаголовокФормы>")`.
...
- `ПолучитьСодержимоеТаблицы` читает до 100 строк. Сужай набор результатов перед чтением больших списков.
...
- `ПолучитьИзмененияОкна` зависит от ранее вызванного `ПолучитьЭлементыАктивногоОкна`. Используй `ПолучитьЭлементыАктивногоОкна` при открытии формы, а затем `ПолучитьИзмененияОкна` при необходимости анализа изменений. Таким способом ты существенно сэкономишь объём информации для анализа.
