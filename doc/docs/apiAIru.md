...
{!id/header.md!}
|элементы|Массив|Иерархический массив визуальных элементов активного окна|

Возвращает дерево элементов только активного окна тестируемого приложения. Элементы массива содержат стандартные поля `Type`, `Name`, `TitleText` и, при наличии дочерних элементов, поле `ChildObjects`.
...
{!id/header.md!}
|изменения|Массив|Изменения элементов активного окна относительно предыдущего снимка|

Возвращает изменения активного окна после предыдущего вызова `ПолучитьЭлементыАктивногоОкна` или `ПолучитьИзмененияОкна`. Если форма открыта впервые или изменилась целиком, возвращает полное дерево элементов.
