...
В текущей версии Тестера встроенный редактор сценариев поддерживает расширенный режим с подсветкой кода. Если в конкретном окружении он работает нестабильно, в настройках можно отключить расширенный редактор и использовать простой текстовый редактор без подсветки.

Для разработки в файловом репозитории можно установить Visual Studio Code, редактируемые там сценарии будут синхронизироваться с Тестером в обе стороны. Подробнее см. в документации [Интеграция с Visual Studio Code](vscode.md). При наборе кода сценария, кроме привычного IntelliSense работают подсказки к методам Тестера; разработанный плагин взаимодействует с Тестером для получения полей тестируемого приложения.
