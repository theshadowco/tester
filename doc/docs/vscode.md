Visual Studio Code (далее vscode) является внешним редактором для работы со сценариями Тестера в файловом репозитории. Встроенный редактор Тестера теперь поддерживает расширенный режим с подсветкой кода, но vscode остается удобным инструментом для массового поиска, замены, навигации по файлам, сравнения изменений и работы с git.

Механизм интеграции Тестера с vscode основан на способности Тестера выгружать сценарии в файлы и загружать их обратно в базу тестов. Специальный плагин и внешняя компонента позволяют общаться Тестеру с vscode для маршрутизации команд между двумя приложениями. Операции выгрузки и загрузки происходят как в ручном так и автоматическом режиме.
