Интерфейс Тестера организован с позиций удобного написания и запуска тестов. При первом запуске, на домашнюю страницу выводится справка по системе (выкачивается из интернета) и открывается основной сценарий, если он задан. В левой части системы находится редактор для ввода кода сценария, справа – дерево сценариев.

Встроенный редактор сценариев поддерживает расширенный режим с подсветкой кода (пока в режиме бета-версии). Если в вашем окружении расширенный редактор работает нестабильно, в настройках Тестера можно отключить расширенный редактор и использовать простой текстовый редактор без подсветки. При написании большого количества сложных тестов по-прежнему можно использовать [интеграцию](vscode.md) с Visual Studio Code, особенно если сценарии хранятся в файловом репозитории.
...
При запуске теста по кнопке `F5` (или командe `Запустить`) Тестер всегда запускает сценарий, установленный как [основной](http://tester.help/#CurrentScenario). Таким образом, вне зависимости от кого, какой сценарий вы в данный момент редактируете, запускаться будет только основной.
