...
Shooter::RawBuffer::RawBuffer ()
		: Buffer ( nullptr ), Copied ( false ), Size ( 0 ) {
}

Shooter::RawBuffer::RawBuffer ( RawBuffer& Parent )
		: Buffer ( Parent.Buffer ), Copied ( false ), Size ( Parent.Size ) {
	Parent.Copied = true;
}
...
Shooter::RawBuffer&
Shooter::RawBuffer::operator= ( Shooter::RawBuffer& Parent ) {
	if ( this == &Parent ) {
		return *this;
	}
...
Shooter::RawBuffer&
Shooter::RawBuffer::operator= ( Shooter::RawBuffer&& Parent ) noexcept {
	Buffer = Parent.Buffer;
	Size = Parent.Size;
	Copied = false;
	Parent.Copied = true;
...
std::optional<Shooter::RawBuffer> Shooter::Take ( const std::wstring& Title ) {
	...
	while ( waiting ) {
		...
		try {
			...
			image = XGetImage ( Monitor, frame, 0, 0, attributes.width,
...
int Shooter::errorHandler ( [[maybe_unused]] Display* Screen,
														XErrorEvent* Error ) {
...
void Shooter::changeState ( Window Frame, bool Revoke, Atom State1,
														Atom State2 ) {
	XEvent event;
	auto& structure = event.xclient;
	structure.type = ClientMessage;
...
std::optional<Window> Shooter::findWindow ( const std::wstring& Pattern ) {
	...
	XGetWindowProperty ( Monitor, DefaultRootWindow ( Monitor ),
											 XInternAtom ( Monitor, "_NET_CLIENT_LIST", false ), 0,
											 ~0L, false, AnyPropertyType, &actualType, &format,
											 &items, &readMore, &data );
	if ( !items || !data ) {
		return std::nullopt;
	}
...
void Shooter::getPng::init () {
	PngStructure = png_create_write_struct ( PNG_LIBPNG_VER_STRING, nullptr,
																					 nullptr, nullptr );
	if ( !PngStructure ) {
...
bool Shooter::locate ( WCHAR* Title ) {
	...
	EnumWindows ( WNDENUMPROC { &nextWindow },
								reinterpret_cast<LPARAM> ( &searchWindow ) );
...
std::unique_ptr<Gdiplus::Bitmap> Shooter::getBitmap () const {
	...
	auto width = ( rect.right - rect.left == 0 )
									 ? GetSystemMetrics ( SM_CXSCREEN )
									 : rect.right - rect.left;
	auto height = ( rect.bottom - rect.top == 0 )
										? GetSystemMetrics ( SM_CYSCREEN )
										: rect.bottom - rect.top;
	auto mybmp = CreateCompatibleBitmap ( desktopdc, width, height );
	auto oldbmp = static_cast<HBITMAP> ( SelectObject ( mydc, mybmp ) );
	BitBlt ( mydc, 0, 0, width, height, desktopdc, rect.left, rect.top,
					 SRCCOPY | CAPTUREBLT );
	SelectObject ( mydc, oldbmp );
	BITMAP bm;
	GetObject ( mybmp, sizeof ( BITMAP ), &bm );
	BitBlt ( mydc, bm.bmWidth, 0, bm.bmWidth, bm.bmHeight, desktopdc, 0, 0,
					 SRCCOPY );
	std::unique_ptr<Gdiplus::Bitmap> result {
			Gdiplus::Bitmap::FromHBITMAP ( mybmp, nullptr ) };
