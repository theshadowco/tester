...
Regex::Regex () : Extender ( L"Regex" ) {
	addFunction ( L"Select", L"Выбрать", 2,
								[ & ] ( tVariant* Params, tVariant* Result ) {
									return select ( Params, Result );
								} );
	addFunction ( L"Test", L"Тест", 2,
								[ & ] ( tVariant* Params, tVariant* Result ) {
									return test ( Params, Result );
								} );
	addFunction ( L"Replace", L"Заменить", 3,
								[ & ] ( tVariant* Params, tVariant* Result ) {
...
bool Regex::select ( tVariant* Params, tVariant* Result ) {
	std::wstring string, query;
	if ( !readWideString ( Params [ 0 ], string, "string" ) ||
			 !readWideString ( Params [ 1 ], query, "query" ) ) {
		return false;
	}
	std::wsmatch match;
...
bool Regex::replace ( tVariant* Params, tVariant* Result ) {
	std::wstring string, query, replacement;
	if ( !readWideString ( Params [ 0 ], string, "string" ) ||
			 !readWideString ( Params [ 1 ], query, "query" ) ||
			 !readWideString ( Params [ 2 ], replacement, "replacement" ) ) {
		return false;
	}
	std::wsmatch match;
	try {
		returnString ( Result,
									 std::regex_replace ( string, Init ( query ), replacement ) );
	} catch ( const std::exception& e ) {
		SetError ( e.what () );
		return false;
	} catch ( ... ) {
