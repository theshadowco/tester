...
namespace Inotify {
Notification::Notification ( const Action& event,
														 const std::filesystem::path& File, bool Directory )
		: Event ( event ), File ( File ), Directory ( Directory ) {};

SystemEvent::SystemEvent ( uint32_t Mask, std::filesystem::path Path,
													 bool Directory )
		: Mask ( Mask ), Path ( std::move ( Path ) ), Directory ( Directory ) {};

Inotify::Inotify ()
		: EventMask ( IN_ALL_EVENTS ),
			IgnoreFolders ( std::vector<std::string> () ), InotifyDescriptor ( 0 ),
			EventBuffer ( EventsLimit * ( EventSize + 16 ), 0 ), PipeReadIndex ( 0 ),
			PipeWriteIndex ( 1 ), Stopped ( false ) {
	...
	if ( epoll_ctl ( EpollDescriptor, EPOLL_CTL_ADD, InotifyDescriptor,
									 &InotifyEpollEvent ) == -1 ) {
		std::stringstream errorStream;
		errorStream << "Can't assign inotify's filedescriptor to epoll: "
								<< strerror ( errno );
		throw std::runtime_error ( errorStream.str () );
	}
	...
	if ( epoll_ctl ( EpollDescriptor, EPOLL_CTL_ADD,
									 StopPipeDescriptor [ PipeReadIndex ],
									 &StopPipeEpollEvent ) == -1 ) {
		std::stringstream errorStream;
		errorStream << "Can't assign pipe filedescriptor to epoll: "
								<< strerror ( errno );
		throw std::runtime_error ( errorStream.str () );
	}
}
...
Inotify::~Inotify () {
	...
	epoll_ctl ( EpollDescriptor, EPOLL_CTL_DEL,
							StopPipeDescriptor [ PipeReadIndex ], nullptr );
...
void Inotify::checkPath ( const std::filesystem::path& Path ) {
	if ( !fs::exists ( Path ) ) {
		throw std::invalid_argument ( "Path not found and will not be monitored: " +
																	Path.string () );
	}
}
...
void Inotify::WatchPath ( const std::filesystem::path& Path ) {
	...
	checkPath ( Path );
	auto descriptor = inotify_add_watch ( InotifyDescriptor,
																				Path.string ().c_str (), EventMask );
	if ( descriptor == -1 ) {
		std::stringstream errorStream;
		...
		if ( errno == ENOSPC ) {
			...
			errorStream << "inotify_add_watch failed with error: "
...
[[maybe_unused]] void
Inotify::ReleasePath ( const std::filesystem::path& File ) {
...
void Inotify::Subscribe ( const std::vector<Action>& Events,
													const EventObserver& EventObserver ) {
...
void Inotify::sendStopSignal () {
	...
	write ( StopPipeDescriptor [ PipeWriteIndex ], buffer.data (),
					buffer.size () );
}
...
void Inotify::attachFolder ( const std::filesystem::path& Folder,
														 const EventObserver* Handler ) {
	( *Handler ) (
			Notification { Action::create | Action::isDir, Folder, true } );
	namespace fs = std::filesystem;
	std::error_code error;
	fs::directory_iterator it (
			Folder, fs::directory_options::follow_directory_symlink, error );
